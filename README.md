...
```bash
mkdir -p "$HOME/.config/livox" && install -m 600 "$HOME/catkin_ws/src/livox_ros_driver/livox_ros_driver/config/livox_network_health.example.json" "$HOME/.config/livox/network_health.json" && nano "$HOME/.config/livox/network_health.json" && python3 "$HOME/catkin_ws/src/livox_ros_driver/livox_ros_driver/livox_ros_driver/scripts/livox_network_health_monitor.py" --config "$HOME/.config/livox/network_health.json" --validate-config
```

如果工位已经存在旧版 `~/.config/livox/network_health.json`，更新脚本会原样保留它；请在更新后确认其中的 `window_seconds` 已从旧值 `10` 改为 `5`，否则该工位仍按旧的十秒窗口运行。新版还建议手动补上 `soft_reboot_settle_seconds: 60`；如果旧文件暂时没有这一项，Driver/monitor 会按60秒默认值运行，但建议显式写入，便于现场审计。旧文件中若仍有 `handle` 字段可以暂时保留，新版会兼容读取但完全忽略它；建议人工删除。不要用模板覆盖包含现场 IP、广播码和网卡名的生产配置。

已有生产配置可以用下面这一行只更新策略字段，不会改动 `targets` 中的广播码、IP 或 `interface`：

```bash
python3 -c 'import json,pathlib; p=pathlib.Path.home()/".config/livox/network_health.json"; d=json.loads(p.read_text()); d.update({"probe_interval_seconds":1,"window_seconds":5,"unstable_failures":2,"unreachable_consecutive_failures":3,"healthy_consecutive_successes":5,"soft_reboot_max_attempts":3,"soft_reboot_interval_seconds":5,"soft_reboot_ack_timeout_seconds":2,"soft_reboot_settle_seconds":60}); p.write_text(json.dumps(d,ensure_ascii=False,indent=2)+"\n")'
```

如果文件尚不存在，先从 `livox_network_health.example.json` 复制一份，再把四台雷达的真实 `broadcast_code` 和固定 IP 填入；`handle` 不要填写，`interface` 通常留空。继电器 `~/.config/livox/power_cycle.json`、Driver 白名单 JSON 和 launch 本次不需要因“软重启稳定等待”改动。

推荐保持 `probe_interval_seconds=1`、`window_seconds=5`、`unstable_failures=2`、`unreachable_consecutive_failures=3`、`healthy_consecutive_successes=5`、`soft_reboot_max_attempts=3`、`soft_reboot_interval_seconds=5`、`soft_reboot_ack_timeout_seconds=2`、`soft_reboot_settle_seconds=60`。5秒内第一次有效丢包显示 `NET_DEGRADED`；5秒内第二次失败，或连续第二次失败的快速路径，立即显示 `NET_UNSTABLE`；连续3次完全无响应显示 `NET_UNREACHABLE`。前5次连续成功探测只用于建立健康基线，期间显示 `NET_UNKNOWN`，不会把正常启动误报成丢包；恢复仍需连续5次成功。网络软重启的5秒 `soft_reboot_interval_seconds` 现在只用于“命令未被接受/发送失败”的短重试；一旦雷达接受了软重启命令，Driver 会进入 `soft_reboot_settle_seconds` 稳定等待，在等待窗口内禁止再次软重启，直到设备信息握手、心跳和点云恢复，或等待窗口到期。根据现场实测，推荐60秒可覆盖约40～48秒的实际恢复时间。若雷达在达到软重启预算后自行恢复，连续5次 `NET_OK` 会清除 Driver 上的网络 `POWER_CYCLE_REQUIRED` 活动锁存，同时保留 `NET_OK` 和历史计数；这种已恢复事件不会再要求继电器 OFF/ON，也不会向 manager 发布自相矛盾的 `NETWORK_RECOVERY_EXHAUSTED + NET_OK` 状态。探测在 `PowerSaving/StandBy` 期间也继续运行，因此看板的 `CURRENT` 不会再用休眠状态掩盖已确认的网络异常；60秒趋势统计仍由 `RECENT 60 SECONDS` 保留。multi launch 中的 `network_health_enable` 默认会启动独立探测进程；配置文件暂时不存在时该进程只记录 WARN 并保持空闲，不会拆掉 Driver。网络看门狗不直接操作继电器：它先按“发送失败短重试、接受后稳定等待”的策略重复软重启，只有配置的软重启预算（推荐3次、每次最多等待60秒）仍未让窗口恢复健康，Driver 才发布 `NETWORK_RECOVERY_EXHAUSTED`，由已经武装的共享电源 manager 执行四路 OFF/ON。
...
脚本会先把工位原文件、更新前仓库版本和差异持久备份到 `~/.local/state/livox-stack-updater/site-config-backups/`，短暂暂存工位修改，再 fast-forward Driver。多雷达 JSON 会字节级原样恢复；multi launch 优先用“现场原文件 / 更新前 HEAD / 更新后上游”做三方合并，使现场参数、新版继电器 include 和 compact 看板入口同时保留。如果文本合并仅因现场参数与新版插入区域重叠而冲突，脚本会以现场 launch 为主体，使用严格结构化后备合并：现场已具有唯一、合法继电器集成时原样保留其 `relay_power_cycle_enable=true/false` 授权和include；完全没有集成时才补入默认 `false` 的安全结构；半集成、重复include或旧inline manager仍拒绝。如果现场 launch 本来含有 `livox_stats_monitor`，还会补入唯一 `monitor_layout` 根参数并让该节点消费 `--layout $(arg monitor_layout)`，默认值为 `compact`。它不会改广播码、remap、其他现场参数，也不会给本来没有看板节点的 headless launch 强行新增看板。只有XML合法、继电器结构唯一，并且已有看板时 `LIVOX_MONITOR_LAYOUT_V1`、`monitor_layout` 和节点参数也都唯一有效，才继续编译；任何身份不明确的结构仍恢复现场旧 launch、保留候选文件并禁止编译和重启。其他任何 tracked 本地修改仍会使更新停止。若进程中断，下次运行会先恢复未完成的配置事务。该选项只接管未暂存修改；若文件已 staged，脚本会停止并要求先取消暂存。继电器现场配置位于仓库外的 `~/.config/livox/power_cycle.json`，更新天然不会覆盖，不需要加入保留列表。
...
roslaunch livox_ros_driver livox_lidar_multi.launch
```
看板默认开启（`monitor` 参数默认 `true`），并使用固定高度的 `monitor_layout:=compact`；驱动日志留在当前终端，看板会**自动弹出一个独立窗口**原地刷新，两者互不干扰。需要旧版完整诊断布局时可运行 `roslaunch livox_ros_driver livox_lidar_multi.launch monitor_layout:=full`。
...
```bash
rosrun livox_ros_driver livox_stats_monitor.py --layout compact
```
> 若提示找不到（旧编译缓存），重新 `catkin_make && source devel/setup.bash` 即可；
> 完整诊断持续刷新：`rosrun livox_ros_driver livox_stats_monitor.py --layout full`；只查看一次完整历史后自动退出：`rosrun livox_ros_driver livox_stats_monitor.py --layout history`。
> 或直接用绝对路径运行：`python3 $(rospack find livox_ros_driver)/livox_ros_driver/scripts/livox_stats_monitor.py --layout compact`
...
0       0.00%           0          0  IDLE          0/3  HEALTHY       --                  start->1/3
1       0.10%           0          1  ACTIVE        1/3  HEALTHY       43.6s@22:08:32      soft 2/3
2       0.00%           0          0  ACTIVE        1/3  VERIFYING     --                  soft 2/3
3          --           0          0  IDLE          0/3  NOT_EXPECTED  --                  start->1/3
==================== ACTION / HISTORY ==================
ID   broadcast_code   current action / process history
0    3WEDH5900101321  none
1    3WEDH7600109791  hist disc=1,pc=1,fault=1,reboot=1
2    3WEDH7600111081  [RECOVER] DATA_VERIFYING
3    1HDDH3200101851  none
POWER-MGR: state=ARMED age=1s mode=armed worker=alive queue=0 obligations=0
POWER-EVENT: none
RELAY: last cycle=none
ASSESS: ACTIVE=fault RECOVERING=repair IDLE=low-power OBSERVE=<10m STABLE=>=10m WATCH/UNSTABLE=trend
RECOVERY: last=duration@first-data; handshake=SDK attempts/60s; details: --layout full; one-shot history: --layout history
```

`full` 模式保留原始Driver诊断、完整时间戳、每个恢复阶段以及最近5次继电器cycle，适合故障排查而非长期盯盘。其输出结构如下：
```
==================== DATA SOURCE ====================
...
==================== MEASUREMENT RECOVERY =========
  (Error budget is per measurement; point-cloud recovery needs 3s confirmation but duration ends at first returned data)
  L2 3WEDJA700100021
    ...
    LAST RECOVERY: 43.6s (duration ends at first data)
      lost at=2026-08-03 08:21:50.184; first data returned=2026-08-03 08:22:33.812; confirmed healthy=2026-08-03 08:22:36.819
...
==================== PROCESS HISTORY =============
  ...
  L2 3WEDJA700100021:
    point-cloud outages=1
      last outage=43.6s; lost at=2026-08-03 08:21:50.184
      first data returned=2026-08-03 08:22:33.812; confirmed healthy=2026-08-03 08:22:36.819
    hardware fault episodes=1; tags=motor+sys; last=2026-08-03 08:21:50
    automatic reboot actions=1, last=2026-08-03 08:21:53
...
上述 `full` 示例从上往下回答：数据源是否仍在更新、实际运行的是哪一对 Driver/SDK、4台里当前有几台故障、每台当前状态、最近60秒发生了什么、本次测量的 Error 重启预算、最近一次真实点云断流从发生到恢复用了多久，以及这个 Driver 进程里以前发生过什么。默认 `compact` 只是把相同证据重新排版，不改变任何判定、计数、自动恢复或继电器安全逻辑。

#### 怎么读看板

`compact` 与 `full` 的信息对应关系如下：顶部一行=`DATA SOURCE + SOFTWARE`，`RECOVERY / RECENT`=`RECENT 60 SECONDS + MEASUREMENT RECOVERY`，`ACTION / HISTORY`=`CURRENT ALERTS + PROCESS HISTORY摘要`，`POWER-MGR/POWER-EVENT/RELAY`=`POWER RECOVERY + 最近一次RELAY HISTORY`。完整多行detail和最近5次cycle只在 `full/history` 中显示。

##### 第一层：compact顶部状态 / full的`DATA SOURCE`

- compact顶部 `Driver=LIVE(age)`（full中为 `DRIVER NOW=LIVE`）才表示下方 Driver 数据仍在实时更新；超过5秒没有新数据会变为 `Driver=STALE` / `DRIVER_STALE severity=CRITICAL`，此时表格只能当最后一次快照。
- compact顶部 `PowerMgr=NOT_SEEN` 表示看板启动以来没有收到manager首帧，可能是继电器功能未启用、正在启动或manager启动失败；收到首帧后直接显示配置授权模式 `ARMED/OBSERVE`，心跳超过30秒变为 `STALE`。full模式仍显示完整 `MANAGER_STALE/CRITICAL`、group和detail。两者都按本机单调时钟计算，不受系统时间跳变或消息内时间戳影响。

##### 第二层：`SOFTWARE` 与 `CURRENT ALERTS`

- compact顶部、full的`SOFTWARE`都显示**正在运行的二进制**内嵌 Driver commit 和固定配套 SDK commit。源码更新后若尚未重启，这里仍会如实显示旧二进制版本；`PINNED` 表示构建时已通过精确SDK SHA校验。
- compact把`CURRENT ALERTS`第一行压到对应雷达的`ACTION / HISTORY`，当前告警永远排在历史摘要之前；`POWER_CYCLE_REQUIRED/STARTUP_MISSING`标为`[CRIT]`，其余故障标为`[ALERT]`，第一批点云回来但尚未连续满3秒标为`[RECOVER] DATA_VERIFYING`。完整session reset、generation、静默和恢复阶段证据仍在full模式。
...
| 列 | 含义 |
|----|------|
| `CURRENT` | 这一秒的真实状态：`NORMAL` / `DATA_VERIFYING` / `NO_DATA` / `DISCONNECTED` / `PLANNED_POWER_CYCLE` / `NORMAL_NO_BROADCAST` / `BROADCAST_RETURNING` / `NORMAL_DROPOUT` / `STARTUP_MISSING` / `WAKE_NO_BROADCAST` / `WAKE_DROPOUT` / `BROADCAST_ONLY` / `HANDSHAKE_STUCK` / `POWER_CYCLE_REQUIRED` / `POWER_SAVING` / `STANDBY` / `CONFIG` / `INIT` / `ERROR`；`DATA_VERIFYING` 表示第一批恢复点云已到达、正在完成连续3秒确认；计划内共享断电不计单机故障趋势，`POWER_CYCLE_REQUIRED` 的告警详情明确标注五种原因之一，未知 SDK 状态显示 `?` 并进入 `ACTIVE` |
...
| `packet_loss / net_loss` | full的`packet_loss`、compact的`net_loss`均表示最近60秒点云网络丢包率，按 `lost / (received + lost)` 计算；窗口内没有点云样本显示 `--`。它不是 Driver 启动以来累计；连接 generation 会显式标记断线清零 |
| `queue_drops / queue_drop` | 最近60秒队列丢包包数：包已到 Driver、但本地队列处理不过来。它和网络丢包是两回事；非0通常指向 CPU、下游订阅者或发布消费瓶颈 |
...
| `handshake_timeouts / handshake` | 最近60秒 SDK 握手 `TIMEOUT` **尝试数**。一次持续卡死期间 SDK 会进行多笔握手，所以它不是独立故障次数，也不是点云UDP丢包率；它单独出现只会把健康雷达提升为 `WATCH`，不会直接判为 `UNSTABLE` |
...
| `RECOVERING` | 当前处于 `BROADCAST_ONLY/CONFIG/INIT`（Config 预算尚未耗尽）、`DATA_VERIFYING`，或已是 `NORMAL` 但这一秒尚未发布点云，尚未达到对应告警条件 |
...
##### 第四层：compact的`RECOVERY / RECENT` / full的`MEASUREMENT RECOVERY`

这个板块把“测量会话内的 Error 升级预算”和“真实点云断流耗时”放在一起。compact每台只占一行：`session / Error / point-cloud / last-recovery / next-error`；full显示完整时间戳和说明：

| 字段 | 含义 |
|------|------|
| `MEASUREMENT SESSION` | `ACTIVE` 表示显式 `PowerSaving→Normal` 已开始本次测量；`PAUSED` 表示进入低功耗时恢复条件尚未满足，预算保留到下次测量；`IDLE` 表示当前没有未结束的测量会话。`explicit/implicit` 分别表示由明确模式命令建立，或 Driver 启动时雷达已在 Normal、第一次 Error 才补建 |
| `ERROR REBOOTS` | 本次测量会话内已经被 SDK 接受的 Error 单台软重启次数，范围 `0/3`～`3/3`；断线、重连和短暂恢复不会清空，只有点云已确认恢复后再正常完成 `Normal→PowerSaving/StandBy` 才结束会话并清零 |
| `POINT-CLOUD` | `NOT_EXPECTED`=当前低功耗/计划切换，不要求出点；`HEALTHY`=当前数据流正常；`OUTAGE`=已经确认或由非计划断连/Error触发的断流正在持续；`VERIFYING`=第一批点云已返回，但还未连续满3秒 |
| `LAST RECOVERY` | 本次 Driver 进程里最近一次已确认的点云恢复耗时。故障起点取断流前**最后一批成功发布点云**，耗时终点取恢复后的**第一批点云**；连续3秒只是确认门槛，不会被加进耗时 |
| `NEXT ESCALATION` | 按当前会话预算直接说明下一次持续3秒的 Error 会执行第几次软重启，还是已经进入 `POWER_CYCLE_REQUIRED reason=ERROR_REBOOT_EXHAUSTED` |

恢复确认严格按下列时间线执行：

```mermaid
flowchart LR
  A["Normal + Sampling<br/>持续发布点云"] --> B["保存最后一批成功发布时刻"]
  B --> C{"非计划断流？<br/>Normal无点云满3秒 / Error / disconnect"}
  C -->|否| A
  C -->|是| D["POINT-CLOUD=OUTAGE<br/>lost_at=断流前最后一批点云"]
  D --> E["第一批点云返回<br/>记录 first_data_returned<br/>CURRENT=DATA_VERIFYING"]
  E --> F{"随后连续发布满3秒？"}
  F -->|中途再次断流| D
  F -->|是| G["确认恢复并写入 POINTCLOUD_RECOVERED<br/>duration=first_data_returned-lost_at"]
  H["PowerSaving / StandBy<br/>计划模式切换"] --> I["排除，不创建点云故障"]
  J["共享继电器计划断电"] --> K["健康伴随雷达排除<br/>原触发雷达已有OUTAGE则保留原起点"]
```

因此，例如 `lost at=08:21:50.184`、`first data returned=08:22:33.812`、`confirmed healthy=08:22:36.819` 时，看板报告的恢复耗时是 `43.628s`，不是 `46.635s`。如果第一批数据回来后3秒内再次断流，只撤销这次候选恢复，原来的 `lost_at` 不变；下一次真正持续稳定的数据返回后才结束同一个 episode。

##### 第五层：compact的`ACTION / HISTORY` / full的`PROCESS HISTORY`
...
| `POWER_CYCLE_REQUIRED: episodes / entries` | `episodes` 是所有原因合计并按故障周期去重的硬恢复 episode；`entries` 是 Driver 已提交进入/重新进入该状态的次数。日志/告警标注 `HANDSHAKE_STUCK / WAKE_DROPOUT / NORMAL_DROPOUT / STARTUP_MISSING / ERROR_REBOOT_EXHAUSTED`。同一 episode 可能在 OFF 前恢复并取消，所以 entries 不能当作独立故障数 |
...
| `point-cloud outages` | 本次 Driver 进程内已经连续确认恢复的真实点云断流次数；若当前仍在断流，会同时显示 `current=ACTIVE/lost at/elapsed`。`last outage` 的耗时截止第一批返回点云，并另外列出3秒确认完成时刻 |
...
默认关闭。开启后，驱动对数据假活、`Error`、Config卡死、握手卡死、显式唤醒掉广播、稳定运行后掉广播和启动缺失使用相互隔离的恢复路径；其中只有五种证据完整的原因会升级到共享继电器硬恢复：
...
**情况 B：测量会话内的 `Error`（如电机故障 `motor=ERR!`）** —— 每次显式 `PowerSaving→Normal` 成功后建立一个测量会话；软重启预算属于整个会话，不属于某次短连接：

| 触发 | 动作 |
|------|------|
| 本次会话第一次 `Error` 连续满 **3 秒** | 请求第1次单台 `RebootDevice`，SDK接受后预算变为 `1/3` |
| 重启产生新 connection generation，随后再次 `Error` 满 **3 秒** | 分别执行第2、3次单台软重启；每次都只增加一次预算 |
| SDK接受重启但 connection generation 没变化 | 保留当前预算并等待约40秒冷却，防止对同一未重启连接形成命令风暴 |
| 已接受3次软重启后，第4次 `Error` 连续满 **3 秒** | 升级 `POWER_CYCLE_REQUIRED reason=ERROR_REBOOT_EXHAUSTED`，交由manager复核后让共享4路断电5秒再上电 |

- 断线、握手、软重启后的新 generation、点云短暂回来，均**不会清空本次测量预算**，避免雷达通过短暂 Normal 状态绕过3次上限
- 点云需要连续发布3秒才确认当前恢复，但恢复耗时截止第一批返回点云；确认恢复本身仍不清预算
- 只有已经确认点云恢复，随后测量流程正常执行 `Normal→PowerSaving/StandBy`，才结束会话并把预算清零；未恢复就进入低功耗则会话暂停，下一次 `PowerSaving→Normal` 继续沿用剩余预算
- Driver 启动时雷达若已经在 Normal，则第一次 Error 会建立一个隐式测量会话，仍使用相同的3次上限
...
下图把五种硬恢复原因分开判定，只在各自证据完整时才汇入同一共享电源组恢复。时间均使用单调时钟；wall-clock 只作为跨进程证据随消息传递。
...
flowchart TD
  ...
  subgraph ERRORSESSION["5. 测量会话内 Error 升级"]
    E0["显式 PowerSaving → Normal 成功<br/>开始测量会话；软重启预算 0/3"] --> ER{"Error 已连续存在满 3 秒？"}
    EI["Driver 启动时雷达已在 Normal<br/>首次 Error 建立隐式测量会话"] --> ER
    ER -->|否 / Error 自行清除| EHEALTH["点云恢复；连续 3 秒后仅确认当前恢复<br/>不清空本次测量的软重启预算"]
    EHEALTH --> ER
    ER -->|是，已接受少于 3 次软重启| ES["单台软重启 attempt + 1<br/>断开 / 握手 / 新 generation 均保留会话预算"]
    ES --> EG{"重启后出现新 connection generation？"}
    EG -->|是| ER
    EG -->|否| EWAIT["同一连接最多等待 40 秒冷却<br/>避免 SDK 只接受命令却未重启时形成命令风暴"]
    EWAIT --> ER
    ER -->|是，已接受 3 次软重启| EPCR["POWER_CYCLE_REQUIRED<br/>reason = ERROR_REBOOT_EXHAUSTED"]
    EHEALTH --> ECLOSE{"点云已连续恢复 3 秒<br/>且正常完成 Normal → PowerSaving / StandBy？"}
    ECLOSE -->|是| ECLEAR["结束测量会话<br/>Error 预算清零"]
    ECLOSE -->|否：未恢复就进入低功耗| EPAUSE["保留预算到下一次测量"]
    EPAUSE --> E0
  end

  subgraph HARD["6. Relay Manager 共享硬恢复（同组 4 台）"]
    PCR --> M0["收到带唯一 episode 身份和 reason 的请求<br/>同组并发事件合并为同一物理端点的一次循环"]
    WPCR --> M0
    NPCR --> M0
    SPCR --> M0
    EPCR --> M0
    M0 --> M1{"原因特定安全复核通过？<br/>HANDSHAKE：required + 广播新鲜 + reset 完成<br/>WAKE：两个 generation 相等；窗内归因；静默 ≥10 秒<br/>NORMAL：两个 generation 相等；此前健康 ≥30 秒；静默 ≥5 秒<br/>STARTUP：handle=255；启动缺失 ≥30 秒；无连接/广播/发布<br/>ERROR：仍 connected + Error + 无点云；同一测量会话已接受 3 次软重启；当前 Error ≥3 秒<br/>其余原因证据必须为 0<br/>armed + 恰好 4 members + 所选通道全部 ON<br/>B0 预检后再收到同 episode/reason 新状态"}
...
| 测量会话内 Error | 状态/动作 |
|------|------|
| 显式 `PowerSaving→Normal` 成功 | 建立本次测量会话，软重启预算从 `0/3` 开始；Driver 启动时雷达已在 Normal，则第一次 Error 自动建立隐式会话 |
| 每次 Error 连续满 3 秒 | 前 3 次分别执行一次单台软重启；软重启、断开、重新握手、新 connection generation 和点云暂时恢复都不清空预算 |
| SDK 接受软重启但 connection generation 没变化 | 同一连接保留 40 秒冷却后才允许下一动作，避免重启命令风暴；一旦出现新 generation，下一次独立 Error 仍只需重新确认 3 秒 |
| 已接受 3 次软重启后，第 4 次 Error 连续满 3 秒 | `POWER_CYCLE_REQUIRED reason=ERROR_REBOOT_EXHAUSTED`；manager 在 OFF 前再次要求目标仍为 connected + Error + 无点云，且会话 ID、3 次预算和 Error 起点仍完全一致 |
| 点云恢复 | 连续发布 3 秒只确认当前恢复，不清空预算；若 relay OFF 前 Error 已消失，当前硬恢复请求立即失效 |
| 点云已连续恢复 3 秒，随后正常完成 `Normal→PowerSaving/StandBy` | 这是唯一清空本次测量 Error 预算的边界；未满足恢复条件就进入低功耗时预算保留到下一次测量 |
...
前面的软恢复只处理**出问题的那一台**。五条硬恢复路径只在各自原因证据完整时汇入共享继电器：handshake/session、wake dropout、normal dropout、白名单启动缺失，以及同一测量会话耗尽 3 次 Error 软重启。五种 reason 互斥，非本 reason 的证据字段必须全部为 0。启动日志仍分别显示自动恢复和握手 session 恢复是否启用。

> ⚠️ 这是驱动**自主重启硬件**的行为，所以默认关闭、需显式开启。无显示器的机器也能用（它和看板无关）。

> **某台 `packet_loss` 偏高 → 排查该台网线/接头/散热；`NO_DATA` → 已连接但无点云；`HANDSHAKE_STUCK` → 广播仍在但控制服务卡住；`WAKE_DROPOUT` → 显式唤醒归因；`NORMAL_DROPOUT` → 此前稳定运行后控制和广播一起消失；`STARTUP_MISSING` → 白名单成员启动宽限内从未出现；`ERROR_REBOOT_EXHAUSTED` → 同一测量会话已经接受 3 次单台软重启，第 4 次 Error 仍持续。`POWER_CYCLE_REQUIRED` 必须继续看 reason。本现场配置 `channels: [1,2,3,4]`，任一有效原因都会用一个掩码让四路及4台雷达一起断电5秒；连接 generation、最新状态复核、冷却和熔断会阻止伴生掉线再次循环。**

#### 可选：原因特定的 `POWER_CYCLE_REQUIRED` 自动继电器硬恢复

这一层只处理五种已确认原因：`HANDSHAKE_STUCK`、`WAKE_DROPOUT`、`NORMAL_DROPOUT`、`STARTUP_MISSING` 和 `ERROR_REBOOT_EXHAUSTED`。没有连续健康/启动宽限/测量会话预算等证据的单次 `DISCONNECTED` 仍不是触发原因。当前继电器1～4路全部只控制这4台雷达，所以该组配置 `channels: [1,2,3,4]`；任一成员通过原因特定复核后，manager用一条A1掩码命令同时操作四路，该组只执行一次OFF/ON。OFF保持采用现场有效 `off_seconds`（新模板5秒，未迁移旧配置可能仍为10秒）。继电器TCP/SQLite运行在独立ROS Python进程中，不进入C++点云收包线程：

1. Driver 在状态首次进入 `POWER_CYCLE_REQUIRED` 时发布带唯一 `event_id`、`recovery_reason` 和原因证据的 `/livox/power_cycle_request`，并以 1 Hz 发布 `/livox/lidar_recovery_state`。normal 请求携带两代相等的 generation、健康起点、归因断线和当前静默起点；startup 请求携带合成 handle 和缺失起点；Error耗尽请求携带测量会话ID、已接受的3次软重启和当前Error起点。
2. `livox_power_cycle_manager.py` 只接受配置中 `power_groups.<组名>.members` 明确列出的 broadcast code，并按五种 reason 严格复核：handshake 要求广播新鲜与 reset 终态；wake 要求同 generation、60 秒归因窗和 10 秒静默；normal 要求同 generation、此前健康至少 30 秒和当前静默至少 5 秒；startup 要求 `handle=255`、启动缺失至少 30 秒且无连接/广播/发布；Error耗尽要求真实handle仍连接、状态仍为Error、没有点云、同一测量会话已接受恰好3次软重启且当前Error至少持续3秒。非本原因证据必须为 0。继电器 B0 预检查后还必须收到触发成员的一帧更新状态；触发者恢复或身份变化时 fail closed，不发 OFF。
...
| 协议确认 | 私有TCP `B0`状态查询接受完整 `CH/CL`；同时兼容CX-5104E-L实机确认的“正确 `CH` + 固定 `AA` 尾字节”（例如全开状态 `... 0D CD AA`）和“正确 `CH` 后完全省略第二校验字节”（例如 `... 0D CD`），两者均保留WARN。新TCP连接首次命令返回精确ASCII `v1.0` 时，manager只对白名单化的这个版本帧在同一连接幂等重发一次，重复版本帧或未知前缀仍失败关闭。实际OFF/ON事务复用一个连接；断线后先换新连接B0，再决定是否需要A1。明确 `ECONNREFUSED` 默认按1秒/3秒有界恢复，其他网络和协议错误不扩展重试。接收端先累计TCP分片并短暂等待第9字节；确认没有尾字节时，只在第一校验、地址、`0D`和四路掩码全部正确时接受8字节帧。错误第一校验、未知非 `AA`尾字节及越界状态一律拒绝。只有固件精确返回 `00 00` 时，才需对单个电源组显式设置 `allow_omitted_status_checksum=true` |
...
| `livox_ros_driver/lds_lidar.h/.cpp` | 模式切换 + 重启 + 批量 Normal 错峰/ACK grace/有界重发 + 每台配置链串行 + 状态机抖动修复 + 广播/握手状态机、session reset、严格 `WAKE_DROPOUT` 归因、测量会话 Error 预算和点云断流时间线 |
| `livox_ros_driver/livox_ros_driver.cpp` | 模式/重启 Service、AsyncSpinner、max_distance 参数、多类自动恢复调度、分层 `livox/lidar_stats` 看板、`MEASUREMENT RECOVERY/DATA_VERIFYING` 展示，以及显式停止 timer/spinner 后的正常关闭 |
| `livox_ros_driver/dashboard_metrics.h` | **新增** — 按 broadcast code 隔离的 60 秒/10 分钟滚动窗口与 `ASSESS` 纯判定逻辑；握手与 wake-dropout episode 分开计数 |
| `livox_ros_driver/measurement_session_policy.h` | **新增** — 每次测量内最多3次 Error 软重启、跨重连保留预算、健康结束测量后才清零，以及耗尽后升级共享硬恢复 |
| `livox_ros_driver/point_cloud_outage_policy.h` | **新增** — 以真实点云发布时刻记录断流/恢复，连续3秒确认但耗时截止第一批返回数据，并排除低功耗与计划维护 |
...
若 Driver 仍已连接该雷达且命令通道可用，可调用软重启 service：`rosservice call /livox_lidar_reboot "{handle: 255}"`（255 = 全部）。对 `HANDSHAKE_STUCK / WAKE_DROPOUT / NORMAL_DROPOUT / STARTUP_MISSING / ERROR_REBOOT_EXHAUSTED` 应使用本章的自动闭环；未满足原因证据的短暂 `DISCONNECTED` 不应手工高频断电。
